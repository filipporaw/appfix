...
const hasRatingPattern = (text: string): number | null => {
  ...
  // Pattern 1c: Pallini Unicode diversi (•, ·, ∙, etc.)
  const unicodeDotPattern = /[•·∙]{1,5}$/;
  const unicodeDotMatch = text.match(unicodeDotPattern);
  if (unicodeDotMatch) {
    const dots = unicodeDotMatch[0];
    return dots.length;
  }
  
  // Pattern 1d: Pallini Unicode nel mezzo del testo
  const unicodeDotPatternMid = /[•·∙]{1,5}/;
  const unicodeDotMatchMid = text.match(unicodeDotPatternMid);
  if (unicodeDotMatchMid) {
    const dots = unicodeDotMatchMid[0];
    if (dots.length > 0) {
      return dots.length;
    }
  }
  ...
  // Pattern 11: Skills con parentesi quadre [4/5], [5/5], etc.
  const bracketPattern = /\[(\d)\/5\]$/;
  ...
  // Pattern 12: Skills con parentesi graffe {4}, {5}, etc.
  const curlyBracketPattern = /\{(\d)\}$/;
  const curlyBracketMatch = text.match(curlyBracketPattern);
  if (curlyBracketMatch) {
    const num = parseInt(curlyBracketMatch[1]);
    if (num >= 1 && num <= 5) {
      return num;
    }
  }
...
const extractSkillName = (text: string): string => {
  ...
  return text
    ...
    .replace(/[●○]{1,5}/, '') // Rimuove cerchi nel mezzo
    .replace(/[•·∙]{1,5}$/, '') // Rimuove pallini Unicode alla fine
    .replace(/[•·∙]{1,5}/, '') // Rimuove pallini Unicode nel mezzo
    ...
    .replace(/\[\d\/5\]$/, '') // Rimuove parentesi quadre con rating
    .replace(/\{\d\}$/, '') // Rimuove parentesi graffe con rating
    .trim();
};
