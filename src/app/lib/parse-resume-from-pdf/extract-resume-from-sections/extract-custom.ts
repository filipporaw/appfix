...
const isCustomSectionTitle = (line: TextItem[]): boolean => {
  ...
  // Controlla se è in grassetto (anche senza essere tutto maiuscolo)
  const isBoldTitle = isBold(textItem);
...
export const extractCustom = (sections: ResumeSectionToLines) => {
  ...
  for (const line of descriptionsLines) {
    ...
    if (lineText) {
      // Salta i titoli di sezione (non li includiamo nelle descrizioni)
      if (isCustomSectionTitle(line)) {
        console.log('🎯 Skipping custom section title:', lineText);
        continue;
      }
